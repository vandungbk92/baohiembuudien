import nguyenvatlieuService from './nguyenvatlieu.service';
import NguyenVatLieu from './nguyenvatlieu.model';
...
export default {
  async create(req, res) {
    try {
      ...
      dsthemmoi.forEach(curr => {
        if(!curr.phieudieutra_id || !curr.loaiphieu_id || !curr.nguyenvatlieu_id){
          isCheck = false
        }
      });
  ...
  async findOne(req, res) {
    try {
      const { id } = req.params;
      const data = await NguyenVatLieu.findOne({is_deleted: false, _id: id})
  ...
  async delete(req, res) {
    try {
      ...
      const data = await NguyenVatLieu.findByIdAndUpdate(id, {is_deleted: true}, { new: true });
  ...
  async update(req, res) {
    try {
      const { id } = req.params;
      const { value, error } = nguyenvatlieuService.validateBody(req.body, 'PUT');
      if (error && error.details) {
        return responseAction.error(res, 400, error.details[0])
      }
      const data = await NguyenVatLieu.findOneAndUpdate({ _id: id }, value, { new: true })
        .populate({path: 'donvi_id', select: 'tendonvi'})
        .populate({path: 'nguyenvatlieu_id', select: 'tennguyenvatlieu'})
