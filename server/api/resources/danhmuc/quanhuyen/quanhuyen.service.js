...
export default {
  validateBody(body, method) {
    let objSchema = {
      maqh: Joi.string().required().label('Mã quận huyện').error((errors) => {
        return {
          template: 'là bắt buộc nhập'
        };
      }),
      tenqh: Joi.string().required().label('Tên quận huyện').error((errors) => {
        return {
          template: 'là bắt buộc nhập'
        };
      }),
      tinhthanh_id: Joi.string().required().label('Mã tỉnh').error((errors) => {
        return {
          template: 'là bắt buộc nhập'
        };
      })
