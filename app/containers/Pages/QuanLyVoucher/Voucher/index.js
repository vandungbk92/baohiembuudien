...
import { add, getById, getAll, delById, updateById } from '@services/quanlyvoucher/voucherService';
...
class Voucher extends Component {
  ...
  columns = [
    ...
    {
      title: 'Mức hội viên yêu cầu',
      dataIndex: 'muchoivien',
      width: 150,
      align: 'center',
    },
    {
  ...
  async getDataFilter() {
    ...
    let queryStr = '';
    queryStr += `${search.tenvoucher ? '&tenvoucher[like]={0}'.format(search.tenvoucher) : ''}`;
  ...
  formatActionCell(value) {
    return <>
      <Link to={URL.VOUCHER_ID.format(value._id)}>
...
<Card size="small" title={<span>
        <UnorderedListOutlined className="icon-card-header"/> &nbsp;Danh sách Voucher
      </span>} 
      md="24" 
      bordered extra={
      <Link to={URL.VOUCHER_ADD}>
...
export default withConnect(Voucher);
