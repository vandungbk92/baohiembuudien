...
import { add, getById, getAll, delById, updateById } from '@services/quanlysologolf/soloService';
...
class Solo extends Component {
  ...
  async getDataFilter() {
    ...
    let queryStr = '';
    queryStr += `${search.solo ? '&solo={0}'.format(search.solo) : ''}`;
  ...
  async handleDelete(value) {
    ...
    if (apiResponse) {
      this.getDataFilter();
      message.success('Xoá số lỗ thành công');
    }
  }
...
<Card size="small" title={<span>
        <UnorderedListOutlined className="icon-card-header"/> &nbsp;Danh sách Số lỗ
      </span>} 
      md="24" 
      bordered extra={
      <Link to={URL.SOLO_ADD}>
...
export default withConnect(Solo);
