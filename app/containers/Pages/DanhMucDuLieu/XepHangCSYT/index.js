...
import { add, getById, getAll, delById, updateById } from '@services/danhmucdulieu/xephangcsytService';
...
class XepHangCSYT extends Component {

  columns = [
    {
      title: 'Tên đơn vị trực thuộc ',
  ...
  async getDataFilter() {
    ...
    let queryStr = ''
    queryStr += `${search.donvitructhuoc ? '&donvitructhuoc[like]={0}'.format(search.donvitructhuoc) : ''}`
  ...
  async handleDelete(value) {
    ...
    if (apiResponse) {
      this.getDataFilter();
      message.success('Xoá Tên đơn vị trực thuộc thành công');
    }
  }
  ...
  handleSaveData = async (data) => {
    let donvitructhuoc = data.donvitructhuoc
    if (!donvitructhuoc.trim()) {
      this.formRef.current.setFieldsValue({ donvitructhuoc: '' });
    ...
    if (_id) {
      ...
      if(apiResponse){
        ...
        message.success('Chỉnh sửa Tên đơn vị trực thuộc thành công');
      }
      ...
      if (apiResponse) {
        ...
        this.getDataFilter();
        message.success('Thêm mới Tên đơn vị trực thuộc thành công');
      }
    }
  }

  formatActionCell(value) {
    console.log(value,'valueupdateee')
    return <Fragment>
  ...
  render() {
    ...
    return <div>
      ...
      <Card size="small" title={<span>
        <UnorderedListOutlined className="icon-card-header"/> &nbsp;Danh sách đơn vị trực thuộc kinh tế của bệnh viện
      ...
      <Modal title={_id ? 'Chỉnh sửa Tên đơn vị trực thuộc' : 'Thêm mới Tên đơn vị trực thuộc'}
              width={1000}
        ...
        <Form ref={this.formRef} id="formModal" name='formModal' autoComplete='off'
          ...
          <Form.Item label="Tên đơn vị trực thuộc" name="donvitructhuoc" hasFeedback labelCol={{ span: 4 }} validateTrigger={['onChange', 'onBlur']}
                     rules={[{ required: true, whitespace: true, message: 'Tên đơn vị trực thuộc không được để trống' }]}>
            <Input placeholder='Tên đơn vị trực thuộc' disabled={loading}/>
          </Form.Item>
...
export default withConnect(XepHangCSYT);