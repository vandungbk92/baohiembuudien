...
import { add, getById, getAll, delById, updateById } from '@services/danhmucdulieu/dmdonviduocdieutraService';
...
class DanhMucDonViDuocDieuTra extends Component {
  ...
  async getDataFilter() {
    ...
    let queryStr = ''
    queryStr += `${search.tendanhmuc ? '&tendanhmuc[like]={0}'.format(search.tendanhmuc) : ''}`
  ...
  async handleDelete(value) {
    ...
    if (apiResponse) {
      this.getDataFilter();
      message.success('Xoá Tên danh mục thành công');
    }
  }
  ...
  handleSaveData = async (data) => {
    let tendanhmuc = data.tendanhmuc
    if (!tendanhmuc.trim()) {
      this.formRef.current.setFieldsValue({ tendanhmuc: '' });
    ...
    if (_id) {
      ...
      if(apiResponse){
        ...
        message.success('Chỉnh sửa Tên danh mục thành công');
      }
      ...
      if (apiResponse) {
        ...
        this.getDataFilter();
        message.success('Thêm mới Tên danh mục thành công');
      }
    }
  }
  ...
  render() {
    ...
    return <div>
      ...
      <Card size="small" title={<span>
        <UnorderedListOutlined className="icon-card-header"/> &nbsp;Danh sách tất cả danh mục
      ...
      <Modal title={_id ? 'Chỉnh sửa Tên danh mục' : 'Thêm mới Tên danh mục'}
            //width={1000}
        ...
        <Form ref={this.formRef} id="formModal" name='formModal' autoComplete='off'
           ...
           <Form.Item label="Mã danh mục" name="madanhmuc" hasFeedback labelCol={{ span: 6 }} validateTrigger={['onChange', 'onBlur']}
                     rules={[{ required: true, whitespace: true, message: 'Mã danh mục không được để trống' }]}>
            <Input placeholder='Mã danh mục' disabled={loading}/>
          </Form.Item>
          <Form.Item label="Tên danh mục" name="tendanhmuc" hasFeedback labelCol={{ span: 6 }} validateTrigger={['onChange', 'onBlur']}
                     rules={[{ required: true, whitespace: true, message: 'Tên danh mục không được để trống' }]}>
            <Input placeholder='Tên danh mục' disabled={loading}/>
          </Form.Item>
...
export default withConnect(DanhMucDonViDuocDieuTra);