...
import { add, getById, getAll, delById, updateById } from '@services/danhmuc/quymobenhvienService';
...
import QuyMoBenhVienModal from 'Pages/DanhMuc/QuyMoBenhVien/QuyMoBenhVienModal';

class QuyMoBenhVien extends Component {

  columns = [
    {
      title: 'Quy mô bệnh viện ',
      dataIndex: 'tenquymo',
      width: 400,
  ...
  async getDataFilter() {
    ...
    let queryStr = '';
    queryStr += `${search.tenquymo ? '&tenquymo[like]={0}'.format(search.tenquymo) : ''}`;
  ...
  async handleDelete(value) {
    ...
    if (apiResponse) {
      ...
      message.success('Xoá quy mô bệnh viện thành công');
    }
  }
  ...
  render() {
    ...
    return <div>
      ...
      <Card size="small" title={<span>
        <UnorderedListOutlined className="icon-card-header"/> &nbsp;Danh sách quy mô bệnh viện
      ...
      <QuyMoBenhVienModal data={this.state.data} showModal={this.state.showModal} getDataAfterSave={this.getDataAfterSave}/>
    </div>;
  }
}
...
export default withConnect(QuyMoBenhVien);
