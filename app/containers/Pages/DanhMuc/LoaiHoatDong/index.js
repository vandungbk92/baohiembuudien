...
import { add, getById, getAll, delById, updateById } from '@services/danhmuc/loaihoatdongService';
...
class LoaiHoatDong extends Component {
  ...
  async getDataFilter() {
    ...
    let queryStr = '';
    queryStr += `${search.tenloaihoatdong ? '&tenloaihoatdong[like]={0}'.format(search.tenloaihoatdong) : ''}`;
  ...
  render() {
    ...
    return <div>
      ...
      <Card size="small" title={<span>
        <UnorderedListOutlined className="icon-card-header"/> &nbsp;Danh sách loại hoạt động
      ...
      <LoaiHoatDongModal data={this.state.data} showModal={this.state.showModal} getDataAfterSave={this.getDataAfterSave}/>
    </div>;
  }
}
...
export default withConnect(LoaiHoatDong);
