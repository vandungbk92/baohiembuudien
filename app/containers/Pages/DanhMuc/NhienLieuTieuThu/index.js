...
import { add, getById, getAll, delById, updateById } from '@services/danhmuc/nhienlieutieuthuService';
...
import NhienLieuTieuThuModal from 'Pages/DanhMuc/NhienLieuTieuThu/NhienLieuTieuThuModal';
import { makeGetMyInfo } from '../../../Layout/HeaderComponent/HeaderProvider/selectors';
import { CONSTANTS } from '@constants';

class NhienLieuTieuThu extends Component {

  columns = [
    {
      title: 'Nhiên liệu tiêu thụ ',
      dataIndex: 'loainhienlieu',
      width: 400,
  ...
  async getDataFilter() {
    ...
    let queryStr = '';
    queryStr += `${search.loainhienlieu ? '&loainhienlieu[like]={0}'.format(search.loainhienlieu) : ''}`;
  ...
  async handleDelete(value) {
    ...
    if (apiResponse) {
      ...
      message.success('Xoá nhiên liệu tiêu thụ thành công');
    }
  }
  ...
  render() {
    ...
    return <div>
      ...
      <Card size="small" title={<span>
        <UnorderedListOutlined className="icon-card-header"/> &nbsp;Danh sách nhiên liệu tiêu thụ
      ...
      <NhienLieuTieuThuModal data={this.state.data} showModal={this.state.showModal} getDataAfterSave={this.getDataAfterSave}/>
    </div>;
  }
}
...
export default withConnect(NhienLieuTieuThu);
