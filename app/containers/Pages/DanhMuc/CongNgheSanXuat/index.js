...
import { add, getById, getAll, delById, updateById } from '@services/danhmuc/congnghesanxuatService';
...
class CongNgheSanXuat extends Component {

  columns = [
    {
      title: 'Công nghệ sản xuất ',
      dataIndex: 'tencongnghesx',
      width: 400,
  ...
  async getDataFilter() {
    ...
    let queryStr = '';
    queryStr += `${search.tencongnghesx ? '&tencongnghesx[like]={0}'.format(search.tencongnghesx) : ''}`;
  ...
  async handleDelete(value) {
    ...
    if (apiResponse) {
      ...
      message.success('Xoá công nghệ sản xuất thành công');
    }
  }
  ...
  render() {
    ...
    return <div>
      ...
      <Card size="small" title={<span>
        <UnorderedListOutlined className="icon-card-header"/> &nbsp;Danh sách công nghệ sản xuất
      ...
      <CongNgheSanXuatModal data={this.state.data} showModal={this.state.showModal} getDataAfterSave={this.getDataAfterSave}/>
    </div>;
  }
}
...
export default withConnect(CongNgheSanXuat);
