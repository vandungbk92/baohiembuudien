...
import { add, getById, getAll, delById, updateById } from '@services/danhmuc/sanphamService';
...
class SanPham extends Component {
  ...
  async getDataFilter() {
    ...
    let queryStr = '';
    queryStr += `${search.tensanpham ? '&tensanpham[like]={0}'.format(search.tensanpham) : ''}`;
  ...
  async handleDelete(value) {
    ...
    if (apiResponse) {
      ...
      message.success('Xoá sản phẩm chính thành công');
    }
  }
  ...
  render() {
    ...
    return <div>
      ...
      <Card size="small" title={<span>
        <UnorderedListOutlined className="icon-card-header"/> &nbsp;Danh sách sản phẩm chính
      ...
      <SanPhamModal data={this.state.data} showModal={this.state.showModal} getDataAfterSave={this.getDataAfterSave}/>
    </div>;
  }
}
...
export default withConnect(SanPham);
