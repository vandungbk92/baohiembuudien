...
import { add, getById, getAll, delById, updateById } from '@services/danhmuc/dmnguyenvatlieuService';
...
class DMNguyenVatLieu extends Component {
  ...
  async getDataFilter() {
    ...
    let queryStr = '';
    queryStr += `${search.tennguyenvatlieu ? '&tennguyenvatlieu[like]={0}'.format(search.tennguyenvatlieu) : ''}`;
  ...
  async handleDelete(value) {
    ...
    if (apiResponse) {
      ...
      message.success('Xoá nguyên, vật liệu thành công');
    }
  }
  ...
  render() {
    ...
    return <div>
      ...
      <Card size="small" title={<span>
        <UnorderedListOutlined className="icon-card-header"/> &nbsp;Danh sách nguyên, vật liệu
      ...
      <DMNguyenVatLieuModal data={this.state.data} showModal={this.state.showModal} getDataAfterSave={this.getDataAfterSave}/>
    </div>;
  }
}
...
export default withConnect(DMNguyenVatLieu);
